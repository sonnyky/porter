...
class PoseRecognizer:
    def __init__(self, pipeline):
        ...
        self.pd_nn.setBlobPath(str(Path("src/models/pose/pose_detection.blob").resolve().absolute()))

        # create landmark model here too for now
        self.lm_nn = pipeline.createNeuralNetwork()
        self.lm_nn.setBlobPath(str(Path("src/models/pose/pose_landmark_lite.blob").resolve().absolute()))
